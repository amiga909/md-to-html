...
async function copyAssets(inputDir, outputDir, assetFiles, log) {
  for (const src of assetFiles) {
    const relPath = path.relative(inputDir, src);
    const dest = path.join(outputDir, relPath);
    await fs.mkdir(path.dirname(dest), { recursive: true });
    await fs.cp(src, dest, { force: true });
    log(`Copied asset: ${relPath}`);
  }
}
...
async function convert(options) {
  ...
  const assetFiles = allFiles.filter((file) => !/\.(md|html)$/i.test(file));
  ...
  if (assets) {
    await copyAssets(inputDir, outputDir, assetFiles, log);
  }
